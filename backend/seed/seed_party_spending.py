"""
Seed Electoral Commission campaign-spending into db.party_spending.

Run from project root:
    python -m backend.seed.seed_party_spending

Source CSV: backend/data/electoral_commission_spending.csv
...
CSV_PATH = DATA_DIR / "electoral_commission_spending.csv"
...
def parse_amount(value: str) -> float:
    """'£1,550.00' -> 1550.0. Returns 0.0 if unparseable."""
    if not value:
        return 0.0
...
def source_url_for(ec_ref: str) -> str:
    return (
        "https://search.electoralcommission.org.uk/Search/CampaignExpenditure"
...
async def main() -> dict:
    ...
    for _, row in df.iterrows():
        ...
        # Spending rows have DateIncurred; prefer DatePaid, fall back to claim
        # date or incurred date so payment_date is always populated.
        payment_date = (
            parse_date(row.get("DatePaid", ""))
            or parse_date(row.get("DateOfClaimForPayment", ""))
            or parse_date(row.get("DateIncurred", ""))
        )

        doc = {
            "party_id":        party_id,
            "supplier":        collapse_ws(row.get("SupplierName", "")),
            "category":        collapse_ws(row.get("ExpenseCategoryName", "")),
            "amount_gbp":      parse_amount(row.get("TotalExpenditure", "")),
            "payment_date":    payment_date,
            "election_period": collapse_ws(row.get("ReportingPeriodName", "")),
            "source_url":      source_url_for(ec_ref),
            "raw_id":          ec_ref,
        }

        await db.party_spending.update_one(
